...
`corey` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/corey/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/corey/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee corey` to
...
python ./.trellis/scripts/task.py archive 00-join-corey
```
